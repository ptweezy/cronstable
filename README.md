...
[![Architectures](https://img.shields.io/badge/arch-amd64%20%7C%20arm64%20%7C%20armv7%20%7C%20armv6%20%7C%20i686%20%7C%20ppc64le%20%7C%20s390x%20%7C%20riscv64%20%7C%20loong64%20%7C%20mips64le-c2185b)](https://github.com/ptweezy/cronstable/releases/latest)
...
`armv7`, `ppc64le`, `s390x`, `riscv64` and `loong64`, plus a musl-only `armv6`
and a glibc-only `mips64le`), macOS (`amd64` and `arm64`, signed and notarized
by Apple), FreeBSD (`amd64` and `arm64`), OpenBSD, NetBSD, illumos (`amd64`)
and Windows (`amd64`, `arm64` and `i686`), plus `.deb` and `.rpm` packages for
the mainstream Linux architectures. The 64-bit glibc builds need only glibc
2.17, so they run on everything from RHEL 7 onward. Python is not required on
