...
def _backend(tmp_path, node="node-a", jsid="v1:jobs", extra=""):
    ...
    # the FilesystemStateBackend state store, while this builds the cluster
    # election FilesystemBackend from cluster YAML with node/jsid knobs.
...
@pytest.fixture
async def fsb_factory(tmp_path):
    """Factory for election backends sharing tmp_path (finding B1).

    ``await fsb_factory(node, jsid=..., extra=...)`` builds a backend and
    starts only its embedded store (never ``start()``, whose
    probe/round/task lifecycle is tested separately); rounds are driven by
    hand with ``await b._renew_once()``.  ``store=False`` skips even that,
    for the tests that drive the full ``start()``/``stop()`` lifecycle
    themselves.  Teardown runs in reverse creation order and mirrors the
    old per-test try/finally sites: the full ``stop()`` when a renew-loop
    task is still running (a full-lifecycle test that failed before its own
    stop), else just the embedded store.
    """
    backends = []

    async def make(node="node-a", jsid="v1:jobs", extra="", *, store=True):
        backend = _backend(tmp_path, node, jsid, extra)
        if store:
    ...
    for backend in reversed(backends):
        with contextlib.suppress(Exception):
            if backend._task is not None:
                await backend.stop()
            else:
                await backend._store.stop()
...
async def test_first_round_elects_and_second_node_defers(
    monkeypatch, fsb_factory
):
    clock = _Clock(monkeypatch)
    a = await fsb_factory("node-a")
    b = await fsb_factory("node-b")
    await a._renew_once()
...
async def test_renew_keeps_fence_takeover_bumps_it(monkeypatch, fsb_factory):
    clock = _Clock(monkeypatch)
    a = await fsb_factory("node-a")
    b = await fsb_factory("node-b")
    await a._renew_once()
...
async def test_challenger_waits_out_the_skew_margin(monkeypatch, fsb_factory):
...
async def test_self_demotion_is_monotonic_and_local(monkeypatch, fsb_factory):
    clock = _Clock(monkeypatch)
    a = await fsb_factory("node-a")
    await a._renew_once()
    assert a.is_leader() is True
...
async def test_quorum_is_a_fixed_deadline(monkeypatch, fsb_factory):
    clock = _Clock(monkeypatch)
    a = await fsb_factory("node-a")
    await a._renew_once()
    assert a.is_quorate() is True
...
async def test_unreadable_store_extends_no_quorum(monkeypatch, fsb_factory):
...
async def test_reboot_ran_round_trips_between_nodes(monkeypatch, fsb_factory):
    _Clock(monkeypatch)
    a = await fsb_factory("node-a", jsid="v1:s1")
    b = await fsb_factory("node-b", jsid="v1:s1")
...
async def test_reboot_ran_scoped_to_job_set_id(monkeypatch, fsb_factory):
    _Clock(monkeypatch)
    a = await fsb_factory("node-a", jsid="v1:s1")
    c = await fsb_factory("node-c", jsid="v1:s2")
    await a.mark_reboot_ran("boot-job")
...
async def test_reboot_ran_appends_are_unioned(monkeypatch, fsb_factory):
...
async def test_takeover_with_failing_ran_refresh_defers_one_shots(
    tmp_path, monkeypatch, fsb_factory
):
...
async def test_follower_failing_ran_read_is_debug_and_throttled(
    monkeypatch, caplog, fsb_factory
):
    ...
    clock = _Clock(monkeypatch)
    ttl = "    ttl: 30\n"
    a = await fsb_factory("node-a", jsid="v1:s1", extra=ttl)
    b = await fsb_factory("node-b", jsid="v1:s1", extra=ttl)
...
async def test_verify_locking_passes_on_a_real_directory(fsb_factory):
    backend = await fsb_factory()
    assert await backend._store.verify_locking() is None


async def test_verify_locking_detects_noop_locks(monkeypatch, fsb_factory):
    backend = await fsb_factory()
...
async def test_start_stop_lifecycle_releases_the_lease(fsb_factory):
    backend = await fsb_factory(store=False)
    await backend.start()
    # the start contract ran one best-effort round: leadership is real
    # before the first spawn pass.  (On an assert failure before the
    # explicit stop below, the fixture teardown runs the full stop().)
    ...
    other = await fsb_factory("node-b")
...
async def test_view_dict_and_lease_detail_shape(monkeypatch, fsb_factory):
    _Clock(monkeypatch)
    a = await fsb_factory("node-a")
    await a._renew_once()
    view = a.view_dict()
...
async def test_renew_loop_survives_store_errors(monkeypatch, fsb_factory):
...
async def test_first_round_elects_and_follower_defers(
    monkeypatch, fsb_factory
):
    clock = _Clock(monkeypatch)
    a = await fsb_factory("node-a")
...
async def test_holding_renew_keeps_fence_no_regain(monkeypatch, fsb_factory):
...
async def test_denied_acquire_confirm_adopts_own_token(
    monkeypatch, fsb_factory
):
...
async def test_observe_read_timeout_changes_nothing(monkeypatch, fsb_factory):
...
async def test_acquire_timeout_changes_nothing(monkeypatch, fsb_factory):
...
async def test_confirm_read_timeout_changes_nothing(monkeypatch, fsb_factory):
...
async def test_start_survives_first_round_failure_unquorate(
    ...
    await backend.start()
    # fixture teardown runs the full stop() (the renew-loop task is live)
...
async def test_renew_loop_reraises_cancellation(monkeypatch, fsb_factory):
    ...
    _Clock(monkeypatch)
    a = await fsb_factory("node-a", store=False)
...
async def test_synced_leader_refresh_failure_logs_debug(
    ...
    clock = _Clock(monkeypatch)
    a = await fsb_factory("node-a", extra="    ttl: 90\n")
