...
# Most tests build their cron through the shared ``stateful_cron`` fixture
# (tests/conftest.py), whose factory and teardown mirror the two helpers
# below.  The locals remain for the sites the fixture cannot express:
# the backend-swap tests and the reload test (stores under paths other than
# plain tmp_path), the GC/manifest tests (mid-test monkeypatch.undo
# interleaving), and the store-error-policy tests whose teardown
# deliberately leaves the broken backend unstopped (``cron.state_backend =
# None``).  Converted restart tests also call _stop_state mid-test:
# stopping the first process there IS the scenario, not cleanup, and the
# fixture's own teardown stays idempotent on an already-stopped cron.
...
async def test_retry_pending_record_written(stateful_cron):
...
async def test_retry_settled_on_success(stateful_cron):
    cron = await stateful_cron(_RETRY_JOB)
    state = JobRetryState(1, 2, 60)
    state.next_delay()
    cron.retry_state["j"] = state
...
async def test_shutdown_cancel_does_not_settle(stateful_cron):
...
async def test_settle_skipped_when_no_retry_was_scheduled(stateful_cron):
...
async def test_job_removed_mid_retry_settles(stateful_cron):
    cron = await stateful_cron(_RETRY_JOB)
...
async def test_abandon_retry_settles_owner_moved(stateful_cron):
    cron = await stateful_cron(_RETRY_JOB)
    state = JobRetryState(1, 2, 60)
    state.next_delay()
    cron.retry_state["j"] = state
...
async def test_abandon_retry_handoff_carries_armed_at(
...
async def test_retry_handoff_armedat_anchors_superseded_guard(stateful_cron):
...
async def test_rearm_pending_retry_after_restart(stateful_cron):
...
async def test_rearm_sleeps_only_remaining_delay(stateful_cron):
...
async def test_rearm_invalidated_on_config_digest_mismatch(stateful_cron):
    cron = await stateful_cron(_RETRY_JOB)
    await _seed_pending(
        cron,
        "j",
...
async def test_rearm_invalidated_when_exhausted(stateful_cron):
    cron = await stateful_cron(_RETRY_JOB)
...
async def test_rearm_invalidated_when_disabled(stateful_cron):
    cron = await stateful_cron(_RETRY_JOB_DISABLED)
    await _seed_pending(
...
async def test_rearm_invalidated_past_starting_deadline(stateful_cron):
    cron = await stateful_cron(_RETRY_JOB_DEADLINE)
...
async def test_rearm_ignores_settled_top_and_garbage(stateful_cron):
    cron = await stateful_cron(_RETRY_JOB)
    await _seed_pending(
        cron, "j", 1, _now_utc() - datetime.timedelta(seconds=5)
    )
...
async def test_rearm_reboot_keepalive_continuity(monkeypatch, stateful_cron):
    ...
    cron = await stateful_cron(_REBOOT_KEEPALIVE_JOB)
...
async def test_rearm_superseded_by_newer_run(stateful_cron):
...
async def test_pause_skip_rows_do_not_settle_a_ladder_across_a_restart(
    ...
    first = await stateful_cron(_RETRY_JOB)
    ...
    second = await stateful_cron(_RETRY_JOB)  # the restart
...
async def test_real_run_under_a_pause_still_settles_the_ladder(stateful_cron):
    ...
    second = await stateful_cron(_RETRY_JOB)
...
async def test_claim_scan_ignores_a_peers_pause_skip_rows(stateful_cron):
...
async def test_peer_claim_folds_pre_ranat_rows_but_not_skips(stateful_cron):
...
async def test_peer_claim_ignores_pause_skip_rows_in_the_ledger(stateful_cron):
...
async def test_rearm_skips_other_hosts_pending(stateful_cron):
...
async def test_rearm_settles_when_retries_disabled(stateful_cron):
    ...
    cron = await stateful_cron(no_retry_yaml)
    await cron.state_backend.append_record(
        "retries/j",
        {
            "kind": "pending",
...
async def test_retry_consume_settles_before_launch(stateful_cron):
...
async def test_cancel_job_retries_swallows_dead_task_error(
    caplog, stateful_cron
):
...
async def test_reboot_marker_dedupes_daemon_restart(
    monkeypatch, stateful_cron
):
    monkeypatch.setattr(platform_mod, "os_boot_id", lambda: "boot-A")
    ...
    cron2 = await stateful_cron(_REBOOT_JOB)
    ...
    monkeypatch.setattr(platform_mod, "os_boot_id", lambda: "boot-B")
    cron3 = await stateful_cron(_REBOOT_JOB)
...
async def test_reboot_redefined_job_runs_again(monkeypatch, stateful_cron):
    monkeypatch.setattr(platform_mod, "os_boot_id", lambda: "boot-A")
    cron = await stateful_cron(_REBOOT_JOB)
    await cron.state_backend.append_record(
        "reboot/r",
...
async def test_launch_gate_still_skips_a_paused_non_reboot_job(stateful_cron):
...
async def test_counters_persist_and_rehydrate(stateful_cron):
    cron = await stateful_cron(_RETRY_JOB)
    cron._record_run("j", _info(second=1, outcome="success"))
    await _drain_state_writes(cron)
    await _stop_state(cron)

    cron2 = await stateful_cron(_RETRY_JOB)
...
async def test_final_counter_snapshot_not_throttled(stateful_cron):
    cron = await stateful_cron(_RETRY_JOB)
...
async def test_trends_from_durable_ledger(stateful_cron):
    cron = await stateful_cron(_RETRY_JOB)
    cron.web_config = {}
    now = _now_utc()
...
async def test_state_periodic_writes_manifest(stateful_cron):
    cron = await stateful_cron(_RETRY_JOB)
...
async def test_list_stream_names_finds_prefix_members_only(stateful_cron):
    cron = await stateful_cron(_RETRY_JOB)
    backend = cron.state_backend
...
async def test_state_metric_families_rendered(stateful_cron):
...
async def test_pause_record_written_and_survives_restart(stateful_cron):
    ...
    cron2 = await stateful_cron(_PAUSE_JOB)
...
async def test_resume_record_clears_pause_on_restart(stateful_cron):
    cron = await stateful_cron(_PAUSE_JOB)
...
async def test_expired_durable_pause_not_rehydrated(stateful_cron):
    cron = await stateful_cron(_PAUSE_JOB)
...
async def test_pause_window_excuses_only_the_slots_inside_it(stateful_cron):
...
async def test_pause_window_leaves_the_pre_pause_backlog_owed(stateful_cron):
...
async def test_catch_up_defers_a_paused_job_and_backfills_on_resume(
    stateful_cron
):
...
async def test_catch_up_pins_backlog_against_held_slot_rows(
...
async def test_pause_whose_durable_append_fails_is_not_reverted(stateful_cron):
