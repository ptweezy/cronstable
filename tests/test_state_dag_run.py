...
async def _make_cron(tmp_path, yaml):
    # Kept (with _teardown below) only for the backend-swap test, which
    # builds crons on two DIFFERENT store paths and tears the peer down
    # mid-test; the shared dag_cron fixture (tests/conftest.py) pins the
    # store to tmp_path, so it cannot serve that one. Every other test in
...
async def test_linear_dag_runs_to_success(tmp_path, dag_cron):
...
async def test_task_retry_then_success(tmp_path, dag_cron):
...
async def test_e2e_fanout_and_xcom(tmp_path, dag_cron):
...
async def test_mapped_upstream_publishes_nothing_expands_empty(
    tmp_path, dag_cron
):
...
async def test_xcom_fanout_parses_bytes_and_offloads_large_payloads(
    ...
    async def publish_and_read(items, run_id):
        ...
        return await cron._dag._read_xcom_list(run_id, "lin", "gen", "items")
...
async def test_mapped_upstream_nonportable_value_does_not_wedge(
    tmp_path, dag_cron
):
...
async def test_e2e_crash_resume(tmp_path, dag_cron):
    yaml = (
        "dags:\n  - name: cr\n    tasks:\n"
        "      - id: a\n        command: 'x'\n        retries: 2\n"
        "      - id: b\n        command: 'x'\n        dependsOn:\n"
...
async def test_approval_gate_blocks_then_resumes(tmp_path, dag_cron):
    yaml = (
        "dags:\n  - name: ap\n    tasks:\n"
        "      - id: a\n        command: 'x'\n"
        "      - id: gate\n        type: approval\n        dependsOn:\n"
...
async def test_approval_reject_skip_cascades(tmp_path, dag_cron):
    # Shares its name with the pure-logic test in test_state_dag.py but
    # covers different machinery: the YAML onReject wiring through the
    # coordinator's approve() reject branch, and the drive resuming after
    # a rejection to cascade the skip durably. Keep both.
    yaml = (
        "dags:\n  - name: rj\n    tasks:\n"
        "      - id: gate\n        type: approval\n        onReject: skip\n"
        "      - id: b\n        command: 'x'\n        dependsOn:\n"
...
async def test_trigger_and_introspection(tmp_path, dag_cron):
    cron = await dag_cron(_LINEAR)
    _set_cmd(cron, "lin", "a", [_PY, "-c", "pass"])
    _set_cmd(cron, "lin", "b", [_PY, "-c", "pass"])
...
async def test_list_dags_caches_terminal_runs(tmp_path, dag_cron):
    # list_dags' rollup must serve immutable terminal runs from cache: after
    # a first pass populates it, a second pass reads no documents at all (no
    # bulk list_documents, no per-run read_document), while the rollup stays
    # correct.
    cron = await dag_cron(_LINEAR)
    _set_cmd(cron, "lin", "a", [_PY, "-c", "pass"])
    _set_cmd(cron, "lin", "b", [_PY, "-c", "pass"])
...
async def test_list_dags_memoizes_the_keys_listing(
    tmp_path, monkeypatch, dag_cron
):
...
async def test_backfill_creates_runs(tmp_path, dag_cron):
    yaml = (
        "dags:\n  - name: bf\n    schedule: '0 * * * *'\n    tasks:\n"
        "      - id: a\n        command: 'x'\n"
    )
...
async def test_scheduled_fire_is_prompt_and_advances(tmp_path, dag_cron):
    ...
    cron._dag._next_logical["sch"] = _utcnow() + datetime.timedelta(minutes=30)
...
async def test_reload_disable_reenable_reseeds_future(tmp_path, dag_cron):
...
async def test_bounded_schedule_exhausts_without_crash(tmp_path, dag_cron):
    ...
    cron._dag._next_logical["once"] = _utcnow() - datetime.timedelta(seconds=1)
    await cron._dag._fire_scheduled(dagrun._now())
...
async def test_awaiting_approval_polls_promptly(tmp_path, dag_cron):
...
async def test_finish_removed_task_is_noop(tmp_path, dag_cron):
...
async def test_decision_on_unowned_run_does_not_pin_wake(tmp_path, dag_cron):
...
async def test_next_wake_delay_prunes_stale_advance_locks(tmp_path, dag_cron):
...
async def test_wake_ignores_inflight_sensor_poke(tmp_path, dag_cron):
...
async def test_failed_completion_record_is_retried(tmp_path, dag_cron):
...
async def test_stale_sensor_poke_completion_retry_is_fenced(
    tmp_path, dag_cron
):
    ...
    _set_cmd(
        ...
        [
            _PY,
            "-c",
            "import os,sys; sys.exit(0 if os.path.exists({!r}) else 1)".format(
                str(flag)
            ),
        ],
    )
...
async def test_noncrontab_schedule_does_not_crash_service(tmp_path, dag_cron):
...
async def test_one_dag_seed_failure_does_not_starve_others(tmp_path, dag_cron):
...
async def test_http_dag_introspection_and_trigger(tmp_path, dag_cron):
    import aiohttp

    cron = await dag_cron(_LINEAR)
    base = await _start_web(cron)
    # the try/finally stays for the web app only (dag_cron cannot stop it);
    # the state teardown itself is the fixture's.
    try:
        _set_cmd(cron, "lin", "a", [_PY, "-c", "pass"])
        _set_cmd(cron, "lin", "b", [_PY, "-c", "pass"])
...
async def test_http_approval_decision_and_backfill(tmp_path, dag_cron):
    import aiohttp
...
async def test_retention_prune_releases_xcom_blobs_to_the_sweep(
    tmp_path, dag_cron
):
    ...
    cron = await dag_cron(_RETAIN_ONE)
...
async def test_gc_removed_dags_grace_and_active_run_protection(
    tmp_path, dag_cron
):
...
async def test_monitored_task_resources_land_in_run_record(tmp_path, dag_cron):
...
async def test_launch_batch_stamps_pids_in_one_rmw(tmp_path, dag_cron):
...
async def test_quiescent_advance_is_single_kept_rmw(tmp_path, dag_cron):
...
async def test_expansion_advance_falls_back_to_two_rmws(tmp_path, dag_cron):
...
async def test_combined_advance_reconciles_crashed_task_inline(
    tmp_path, dag_cron
):
...
async def test_failed_batch_pid_write_does_not_fail_tasks(tmp_path, dag_cron):
...
async def test_one_launch_failure_does_not_skip_the_batch(tmp_path, dag_cron):
...
async def test_catch_up_replays_missed_slots(tmp_path, dag_cron):
...
async def test_catch_up_honours_starting_deadline(tmp_path, dag_cron):
    cron = await dag_cron(_HOURLY)
...
async def test_catch_up_cap_truncation_is_loud(
    ...
    warned = [
        r for r in caplog.records if "missed at least 2 runs" in r.getMessage()
    ]
...
async def test_catch_up_applies_the_dag_jitter_offset(
    tmp_path, monkeypatch, dag_cron
):
...
async def test_deferred_catch_up_checkpoint_survives_a_restart(
    ...
    for task in list(cron._dag._catchup_tasks):
        task.cancel()
    await asyncio.gather(*cron._dag._catchup_tasks, return_exceptions=True)
...
async def test_deferred_catch_up_closes_its_checkpoint(
    tmp_path, monkeypatch, dag_cron
):
...
async def test_catch_up_without_prior_run_is_noop(tmp_path, dag_cron):
    cron = await dag_cron(_HOURLY)
    dagcfg = cron.cron_dags["cu"]
    await cron._dag._catch_up(
...
async def test_release_swallows_backend_errors(
    ...
    # Same name as a test in test_state_job_api.py, but not a duplicate:
    # DagCoordinator._release (dagrun.py) has its own swallow, separate
    # from JobLockManager._release_hold (jobapi.py). Keep both.
    cron = await dag_cron(_LINEAR)
    ref = ("lin", "rk1")
...
async def test_try_own_shortcuts_and_failures(tmp_path, monkeypatch, dag_cron):
    cron = await dag_cron(_LINEAR)
...
async def test_advance_releases_run_of_removed_dag(tmp_path, dag_cron):
    cron = await dag_cron(_LINEAR)
    ref = ("ghost", "rk1")
...
async def test_adopt_orphans_keys_pass_and_full_pass(tmp_path, dag_cron):
    cron = await dag_cron(_LINEAR)
    _set_cmd(cron, "lin", "a", [_PY, "-c", "pass"])
    _set_cmd(cron, "lin", "b", [_PY, "-c", "pass"])
...
async def test_compute_wake_prefers_due_retry(tmp_path, dag_cron):
    cron = await dag_cron(_LINEAR)
    spec = cron.cron_dags["lin"].spec
    now = 1000.0
    body = {
...
async def test_xcom_for_run_inlines_and_flags_values(tmp_path, dag_cron):
    ...
    res = await cron._dag.xcom_for_run("xc", "r1", max_value_bytes=4)
    assert res["dag"] == "xc"
...
async def test_xcom_for_run_truncates_beyond_max_entries(tmp_path, dag_cron):
    cron = await dag_cron(_XC_YAML)
    run_id = await _mint_run(cron, "r1")
    scope = dag.xcom_scope("xc", str(run_id))
    for i in range(3):
...
async def test_xcom_for_run_unknown_dag_or_run_returns_none(
    tmp_path, dag_cron
):
    cron = await dag_cron(_XC_YAML)
...
async def test_xcom_for_run_without_run_id_returns_empty(tmp_path, dag_cron):
    cron = await dag_cron(_XC_YAML)
    await _mint_run(cron, "r1")
...
async def test_xcom_for_run_backend_hiccup_degrades(
    tmp_path, monkeypatch, dag_cron
):
    cron = await dag_cron(_XC_YAML)
    run_id = await _mint_run(cron, "r1")
...
async def test_xcom_for_run_swept_blob_has_no_value(
    tmp_path, monkeypatch, dag_cron
):
    cron = await dag_cron(_XC_YAML)
    run_id = await _mint_run(cron, "r1")
...
# The definitively-answered payload shapes, one row per former standalone
# test (the deleted test_read_xcom_list_guards superset walked the same
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            b'["a", "b", "c"]', ["a", "b", "c"], id="parses-published-list"
        ),
        # upstream succeeded but never published this key -> no items to map
        pytest.param(None, [], id="absent-key-is-empty"),
        # junk payloads definitively map to an empty fan-out
        pytest.param(b"this is not json", [], id="invalid-json-is-empty"),
        pytest.param(b'{"a": 1}', [], id="non-list-json-is-empty"),
    ],
)
async def test_read_xcom_list_definitive_payloads(dag_cron, payload, expected):
    cron = await dag_cron(_XC_YAML)
    if payload is not None:
        await jobstate.artifact_put(
            cron.state_backend,
            dag.xcom_scope("xc", "run-x"),
            dag.xcom_name("gen", "items"),
            payload,
        )
...
async def test_read_xcom_list_broken_record_reads_is_none(
    monkeypatch, dag_cron
):
    # Kept from the deleted test_read_xcom_list_guards superset (its other
    # branches are the parametrize rows above): a store that cannot be read
    # leaves the fan-out unknown (None)
    cron = await dag_cron(_XC_YAML)
    scope = dag.xcom_scope("xc", "run-x")
    await jobstate.artifact_put(
        cron.state_backend, scope, dag.xcom_name("gen", "items"), b"[1, 2, 3]"
    )
    read = cron._dag._read_xcom_list
    assert await read("run-x", "xc", "gen", "items") == [1, 2, 3]
    _break_record_reads(
        monkeypatch,
        cron.state_backend,
        jobstate.ARTIFACT_STREAM_PREFIX + scope,
    )
    assert await read("run-x", "xc", "gen", "items") is None


async def test_read_xcom_list_oversized_blob_is_empty(
    tmp_path, monkeypatch, dag_cron
):
...
async def test_read_xcom_list_timeout_is_none(tmp_path, monkeypatch, dag_cron):
    cron = await dag_cron(_XC_YAML)
...
async def test_read_xcom_list_missing_blob_is_empty(
    tmp_path, monkeypatch, dag_cron
):
    cron = await dag_cron(_XC_YAML)
...
async def test_read_xcom_list_store_error_is_none(
    tmp_path, monkeypatch, dag_cron
):
    cron = await dag_cron(_XC_YAML)
...
async def test_bulk_rollup_builds_cache_and_rollup(tmp_path, dag_cron):
    cron = await dag_cron(_XC_YAML)
    await _mint_run(cron, "r1")
...
async def test_dag_run_rollup_reads_and_caches(tmp_path, dag_cron):
    cron = await dag_cron(_XC_YAML)
    await _mint_run(cron, "r1")
    await _mint_run(cron, "r2")
...
async def test_dag_run_rollup_degrades_on_keys_error(
    tmp_path, monkeypatch, dag_cron
):
    cron = await dag_cron(_XC_YAML)
    await _mint_run(cron, "r1")
...
async def test_bulk_rollup_degrades_on_error(tmp_path, monkeypatch, dag_cron):
    cron = await dag_cron(_XC_YAML)
    await _mint_run(cron, "r1")
...
async def test_dag_run_rollup_prunes_gone_cache_keys(tmp_path, dag_cron):
    cron = await dag_cron(_XC_YAML)
    await _mint_run(cron, "r1")
...
async def test_dag_run_rollup_missing_body_is_skipped(
    tmp_path, monkeypatch, dag_cron
):
    cron = await dag_cron(_XC_YAML)
    await _mint_run(cron, "r1")
...
async def test_forget_clears_every_per_store_cache(tmp_path, dag_cron):
    cron = await dag_cron(_XC_YAML)
    await _mint_run(cron, "r1")
    await cron._dag._dag_run_rollup(cron.state_backend, "xc")
...
async def test_adopt_one_dag_keys_only_then_full(tmp_path, dag_cron):
    cron = await dag_cron(_LINEAR)
    _set_cmd(cron, "lin", "a", [_PY, "-c", "pass"])
    _set_cmd(cron, "lin", "b", [_PY, "-c", "pass"])
    dagcfg = cron.cron_dags["lin"]
...
async def test_lease_usable_live_stale_and_taken_over(
    tmp_path, monkeypatch, dag_cron
):
    yaml = (
        "dags:\n  - name: lu\n    tasks:\n"
...
async def test_prepare_task_run_env_and_secrets(tmp_path, caplog, dag_cron):
...
async def test_flush_run_completions_drops_removed_task_and_dag(
    tmp_path, dag_cron
):
    cron = await dag_cron(_LINEAR)
    dagcfg = cron.cron_dags["lin"]
...
async def test_service_skips_when_a_pass_is_in_flight_or_nothing_due(
    tmp_path, dag_cron
):
    cron = await dag_cron(_LINEAR)
    d = cron._dag
...
async def test_run_service_cadence_branches(tmp_path, dag_cron):
    cron = await dag_cron(_LINEAR)
    d = cron._dag
...
async def test_run_service_isolates_and_propagates_errors(
    tmp_path, monkeypatch, dag_cron
):
    cron = await dag_cron(_LINEAR)
    d = cron._dag
...
async def test_next_wake_delay_prunes_unowned_wake(tmp_path, dag_cron):
    cron = await dag_cron(_LINEAR)
    d = cron._dag
...
async def test_seed_dag_catch_up_error_is_isolated(
    tmp_path, monkeypatch, dag_cron
):
    yaml = (
        "dags:\n  - name: cs\n    schedule: '0 * * * *'\n"
...
async def test_fire_scheduled_cluster_deny_and_cancel(
    tmp_path, monkeypatch, dag_cron
):
    yaml = (
        "dags:\n  - name: fs\n    schedule: '* * * * *'\n    tasks:\n"
...
async def test_fire_forward_gap_hands_the_gap_to_onmissed(
    ...
    assert (
        now_dt < d._next_logical["fh"] <= now_dt + datetime.timedelta(hours=1)
    )
...
async def test_catch_up_deadline_not_binding(tmp_path, dag_cron):
    cron = await dag_cron(_HOURLY)
    dagcfg = cron.cron_dags["cu"]
    sched = dagcfg.schedule_job
...
async def test_catch_up_no_missed_slots_returns(tmp_path, dag_cron):
    cron = await dag_cron(_HOURLY)
    dagcfg = cron.cron_dags["cu"]
...
async def test_try_own_without_backend_fails_closed(tmp_path, dag_cron):
    cron = await dag_cron(_LINEAR)
    dagcfg = cron.cron_dags["lin"]
    backend = cron.state_backend
    cron.state_backend = None
...
async def test_renew_loop_exits_when_ref_no_longer_owned(
    tmp_path, monkeypatch, dag_cron
):
    cron = await dag_cron(_LINEAR)
    _instant_sleep(monkeypatch)
...
async def test_renew_loop_propagates_cancel(tmp_path, monkeypatch, dag_cron):
    cron = await dag_cron(_LINEAR)
    ref = ("lin", "rk-cancel")
    cron._dag._owned[ref] = Lease("dagadvance/lin/rk-cancel", "h#1", 1, 9e18)
...
async def test_adopt_one_dag_keys_timeout_returns(
    tmp_path, monkeypatch, dag_cron
):
    cron = await dag_cron(_LINEAR)
    dagcfg = cron.cron_dags["lin"]
...
async def test_adopt_one_dag_read_timeout_returns(
    tmp_path, monkeypatch, dag_cron
):
    cron = await dag_cron(_LINEAR)
    d = cron._dag
...
async def test_advance_owned_advances_due_ref(tmp_path, dag_cron):
    cron = await dag_cron(_LINEAR)
    _set_cmd(cron, "lin", "a", [_PY, "-c", "pass"])
    _set_cmd(cron, "lin", "b", [_PY, "-c", "pass"])
    run_key = await cron._dag.trigger_run("lin")
...
async def test_advance_locked_backs_off_on_unusable_lease(tmp_path, dag_cron):
    yaml = (
        "dags:\n  - name: ul\n    tasks:\n"
        "      - id: a\n        command: 'x'\n"
    )
...
async def test_lease_usable_propagates_cancel(tmp_path, monkeypatch, dag_cron):
    yaml = (
        "dags:\n  - name: lc\n    tasks:\n"
        "      - id: a\n        command: 'x'\n"
    )
    cron = await dag_cron(yaml)
    ref = ("lc", "rk")
    lease = Lease(cron._dag._lease_name(ref), "h#1", 1, dagrun._now() - 5.0)
...
async def test_do_advance_expansion_second_rmw_edge_cases(tmp_path, dag_cron):
...
async def test_launch_cancel_propagates_out_of_advance(
    tmp_path, monkeypatch, dag_cron
):
    yaml = (
        "dags:\n  - name: lp\n    tasks:\n"
...
async def test_empty_fanout_terminalises_after_claim(tmp_path, dag_cron):
    yaml = (
        "dags:\n  - name: ez\n    tasks:\n"
        "      - id: gen\n        command: 'x'\n"
        "      - id: work\n        command: 'x'\n        dependsOn:\n"
...
async def test_on_task_finished_unknown_dag_is_noop(tmp_path, dag_cron):
    cron = await dag_cron(_LINEAR)
...
async def test_flush_completions_propagates_cancel(
    tmp_path, monkeypatch, dag_cron
):
    cron = await dag_cron(_LINEAR)
    ref = ("lin", "rk-fc")
...
async def test_flush_run_completions_propagates_cancel(tmp_path, dag_cron):
    cron = await dag_cron(_LINEAR)
    dagcfg = cron.cron_dags["lin"]
    run_key = "manual-frc"
    assert await cron._dag._create_doc(dagcfg, run_key, None, "manual")
...
async def test_finish_task_queues_on_error_and_propagates_cancel(
    tmp_path, dag_cron
):
    cron = await dag_cron(_LINEAR)
    dagcfg = cron.cron_dags["lin"]
...
async def test_queue_completion_backs_off_on_repeat(tmp_path, dag_cron):
    cron = await dag_cron(_LINEAR)
    d = cron._dag
    ref = ("lin", "rk-q")
    d._queue_completion(
...
async def test_retry_completions_skips_not_due_and_removed_dag(
    tmp_path, dag_cron
):
    cron = await dag_cron(_LINEAR)
    d = cron._dag
...
async def test_backfill_unscheduled_or_unknown_dag(tmp_path, dag_cron):
    cron = await dag_cron(_LINEAR)  # lin has no schedule
    res = await cron._dag.backfill(
...
async def test_get_run_unknown_dag_is_none(tmp_path, dag_cron):
    cron = await dag_cron(_LINEAR)
...
async def test_bulk_rollup_propagates_cancel(tmp_path, monkeypatch, dag_cron):
    cron = await dag_cron(_XC_YAML)
...
async def test_dag_run_rollup_propagates_cancel_on_keys_and_read(
    ...
    monkeypatch.setattr(cron.state_backend, "list_document_keys", _cancel_keys)
    with pytest.raises(asyncio.CancelledError):
        await cron._dag._dag_run_rollup(cron.state_backend, "xc")
    monkeypatch.undo()
...
async def test_gc_runs_no_backend_isolation_and_cancel(
    tmp_path, monkeypatch, dag_cron
):
    cron = await dag_cron(_LINEAR)
    d = cron._dag
...
async def test_gc_one_dag_skips_empty_run_key(tmp_path, monkeypatch, dag_cron):
...
async def test_delete_run_swallows_prune_error(
    tmp_path, monkeypatch, dag_cron
):
    cron = await dag_cron(_LINEAR)
    dagcfg = cron.cron_dags["lin"]
...
async def test_shutdown_cancels_live_service_task(tmp_path, dag_cron):
    cron = await dag_cron(_LINEAR)
...
async def test_forget_cancels_live_service_task(tmp_path, dag_cron):
    cron = await dag_cron(_LINEAR)
