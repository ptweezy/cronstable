...
#
#   $1          the requirement generated from pyproject.toml into the
#               Dockerfile by scripts/generate_build_files.py.
...
#   - the requirement is an argument here; the pyinstaller script reads its
#     generated requirements file. Both derive from pyproject.toml.
#   - both use pyinstaller/verify_extra.py; the image copies it beside this
#     script below the dependency layers, so probe edits keep their cache.
...
orjson_ok() {
    /opt/venv/bin/python /tmp/deps/verify_extra.py orjson
}
