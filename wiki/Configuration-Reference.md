...
| `reach.keyFile` | `Str` | required within `reach` | The node's remote-access identity file (two key seeds and the admission salt), created with mode `0600` on first start. The `reach` block requires a bearer token (`authToken` or an `authTokens` entry), PyNaCl (the `push` extra), and no `cluster:` section; each is a `ConfigError` at load. See [remote access](Remote-Access). |
| `reach.relay` | `Str` | the origin of `push.relay.url` | The relay origin the daemon dials: `https` only, or `http` to a loopback host; scheme and host, with no path, query, or fragment. Userinfo becomes Basic authentication on the WebSocket upgrade; the daemon redacts it from logs. Without it, the block requires a `push:` section to derive the origin from. |
| `reach.heartbeat` | `Float` | `30` | Seconds between WebSocket pings to the relay, 5 to 300. |
