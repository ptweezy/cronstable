...
a backend, a clock, or a subprocess, and the cron wiring (:mod:`yacron2.cron`)
is a thin driver that persists the results through
:meth:`yacron2.state.StateBackend.mutate_document`.
...
ENV_DAG_NAME = "YACRON2_DAG_NAME"
ENV_DAG_RUN_ID = "YACRON2_DAG_RUN_ID"
ENV_DAG_RUN_KEY = "YACRON2_DAG_RUN_KEY"
ENV_DAG_TASK = "YACRON2_DAG_TASK"  # the base task id
ENV_DAG_TASKKEY = "YACRON2_DAG_TASKKEY"  # the instance key (id or id#index)
ENV_DAG_MAP_INDEX = "YACRON2_DAG_MAP_INDEX"
ENV_DAG_MAP_ITEM = "YACRON2_DAG_MAP_ITEM"
...
class TaskSpec:
    ...
    (command, env, timeouts) lives in a sibling ``yacron2.config.JobConfig``
...
def validate_graph(spec: DagSpec) -> None:
    ...
    bad DAG is a :class:`~yacron2.config.ConfigError` at load, not a runtime
    hang.
    """
