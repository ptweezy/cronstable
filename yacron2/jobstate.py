...
memory) and live in :mod:`yacron2.jobapi`.
...
(:mod:`yacron2.jobapi`) and the offline CLI (:mod:`yacron2.jobcli`) are two
...
class JobStateError(Exception):
    ...
    ``status`` mirrors HTTP so :mod:`yacron2.jobapi` can answer with it
...
def _now() -> float:
    ...
    A separate seam from :func:`yacron2.state._now` (the store's lease clock)
...
async def idempotency_claim(
    ...
    *,
    ...
    Honest bound: like every yacron2 coordination primitive this is
