...
lock|artifact|idempotent|secret`` command (see :mod:`yacron2.jobcli`) is a
...
* a **mutex / semaphore** is a lease that must be *renewed* for as long as the
  ...
  (:mod:`yacron2.cron`), and a short-lived ``yacron2 lock`` subprocess cannot;
...
functions over the backend (:mod:`yacron2.jobstate`); this module adds the
...
# YACRON2_JOB_NAME deliberately matches the key ShellReporter already sets.
ENV_URL = "YACRON2_STATE_URL"
ENV_TOKEN = "YACRON2_STATE_TOKEN"
ENV_RUN_ID = "YACRON2_RUN_ID"
ENV_JOB_NAME = "YACRON2_JOB_NAME"
ENV_ATTEMPT = "YACRON2_ATTEMPT"
ENV_SCHEDULED_AT = "YACRON2_SCHEDULED_AT"
ENV_HOST = "YACRON2_HOST"
...
def run_environment(ctx: RunContext, base_url: str) -> Dict[str, str]:
    """The ``YACRON2_*`` env the daemon injects for one run.
...
class JobLockManager:
    ...
    machinery in :mod:`yacron2.cron`; it is the same lease API and the same
...
class JobStateAPI:
    ...
    :class:`yacron2.cron.Cron`, which registers a run before launching it and
    ...
    async def _h_artifact_get(
        ...
        return web.Response(
            ...
            headers={
                "X-Yacron2-Sha256": str(record.get("sha256", "")),
                "X-Yacron2-Size": str(record.get("size", len(data))),
            },
        )
