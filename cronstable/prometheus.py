"""Native Prometheus metrics for the cronstable web API (``GET /metrics``).

The pull-side sibling of :mod:`cronstable.statsd`: instead of pushing per-run
...
#: Every integral value from 0 to 4095, rendered.  An int, an integral
...
def render_families(
    ...
    for family in families:
        ...
        # The per-sample line is built inline: this is the innermost loop
        # of a scrape (tens of thousands of samples on a large fleet),
        # where a helper's call frame and its (name, block) tuple cost
        # more than the two-line body they save.  Formatting is the same
        # as iter_family_samples': same stem, block builder, and
        # format_value.
        append = out.append
        for suffix, labels, value in family.samples:
            block = block_for(labels) if labels else ""
            # one f-string build per line: chained concatenation allocates
            # an intermediate string per operator
...
class PrometheusMetrics:
    ...
    Owned by :class:`cronstable.cron.Cron` for the life of the process, so
    ...
    def _state_families(self, cron: "Cron") -> list[MetricFamily]:
        ...
        if lock.get("acquisitions"):
            acquisitions = MetricFamily(
                "cronstable_state_lock_acquisitions",
                "counter",
            ...
            lock_wait = MetricFamily(
                "cronstable_state_lock_wait_seconds",
                "counter",
        ...
        if throttle.get("count"):
            throttled = MetricFamily(
                "cronstable_state_throttled_ops",
                "counter",
            ...
            throttle_wait = MetricFamily(
                "cronstable_state_throttle_wait_seconds",
                "counter",
        ...
        if workers:
            inflight = MetricFamily(
                "cronstable_state_workers_inflight",
                "gauge",
            ...
            capacity = MetricFamily(
                "cronstable_state_workers_capacity",
                "gauge",
                "Worker-slot concurrency cap per lane.",
            )
            peak = MetricFamily(
                "cronstable_state_workers_peak",
                "gauge",
    ...
    def _daemon_families(self, cron: "Cron") -> list[MetricFamily]:
        ...
        start = MetricFamily(
            "cronstable_start_time_seconds",
            "gauge",
            "Unix time this cronstable process started.",
        )
        ...
        if self._last_reload_ok is not None:
            reload_ok = MetricFamily(
                "cronstable_config_last_reload_successful",
                "gauge",
        ...
        if self._last_reload_success_time is not None:
            reload_time = MetricFamily(
                "cronstable_config_last_reload_success_timestamp_seconds",
                "gauge",
    ...
    def _job_families(self, cron: "Cron") -> list[MetricFamily]:
        ...
        last_run_max_rss = MetricFamily(
            "cronstable_job_last_run_max_rss_bytes",
            "gauge",
        ...
        for name, job_config in cron.cron_jobs.items():
            ...
            info.add(
                {
                    "job_name": name,
                    # schedule_str's string, precomputed on the JobConfig
    ...
    def _cluster_families(self, cron: "Cron") -> list[MetricFamily]:
        ...
        if elect_leader or self._quorum_transitions:
            quorum_transitions = MetricFamily(
                "cronstable_cluster_quorum_transitions",
                "counter",
