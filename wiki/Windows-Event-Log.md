...
at configuration load, because a source names a key underneath a log rather
than a log or a path. The check applies only to blocks that are enabled,
because `source` carries its default into every report block of every hook.
...
8000. A field cut short ends with `...[truncated]`. The cap is arithmetic
rather than a runtime trim because of what happens otherwise: when the
combined vector is too long, `ReportEventW` writes no record at all, so an
uncapped reporter would drop precisely the alerts for the jobs that
produced the most output.

## The "description not found" preamble

Because cronstable does not register its event source, Event Viewer has no
...
Registering a source is an HKLM write, which needs Administrator. Without a
message DLL to point `EventMessageFile` at, registering provides no benefit.
Building one means an `mc.exe` and `rc.exe` step per architecture, shipping
a second binary beside the executable, and the code-signing question that
binary would raise. Therefore, cronstable writes as an unregistered source
and documents the preamble rather than asking every install for
Administrator to remove a cosmetic line.

If you have your own message DLL, you can register the source yourself. The
...
IDs have to match the earlier table.
...
but anything written here is visible to anyone who can open Event Viewer on
the host. Turning `includeOutput` on is a decision to publish that job's
output locally.

**Queued records are not durable.** Writes are handed to a background thread
so a busy Event Log service can never delay a job's completion handling. A
graceful shutdown drains that queue with a five-second bound. An abrupt
process exit drops whatever had not yet reached the service.
...
per-run outcomes. An unattended deployment usually benefits from both. The
standard library's `logging.handlers.NTEventLogHandler` is not an
