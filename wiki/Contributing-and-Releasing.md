...
`pyproject.toml` owns dependency declarations. After a dependency update, run `python scripts/generate_build_files.py`; `tox-static` checks that the derived files match before dependent builds start. When Dependabot updates `requirements_min.txt`, first promote the accepted pins to the corresponding declarations in `pyproject.toml`, including matching extras, then regenerate. The Nix flake reads its build and runtime dependency names directly from the same declarations. Its small set of source overrides covers packages that nixpkgs has not updated yet, preserves upstream build and test recipes, and automatically yields to newer nixpkgs versions. The advisory `nix` job follows `tox-static`, checks the package's runtime requirements and imports, and smoke-tests the installed CLI.
