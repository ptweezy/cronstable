...
`tox-uv`, and uv can fetch the 3.10–3.14 interpreters the matrix needs):
...
`tox` drives all CI checks (`tox.ini`). The default `envlist` is `py3{10,11,12,13,14}-{windows,posix}, lint, mypy, bandit, openapi`.
...
2. A **manual `workflow_dispatch`** run, choosing the bump level from a dropdown (`minor` default, or `major` / `patch`). The same form has a `perf` dropdown (`gate` default, `accept`, `ignore`) that overrides the performance gate; see [performance benchmarks](Performance-Benchmarks#overriding-the-gate).
...
4. **Binary builds** (run in parallel after the required tests and preflight; the publish jobs need them, so a broken build fails the run instead of producing a half-finished release). Each job pins `pyinstaller==6.22.1`, installs the project to bake `SETUPTOOLS_SCM_PRETEND_VERSION` (the computed version) into `cronstable/version.py`, runs `pyinstaller pyinstaller/cronstable.spec`, and smoke-tests the bundle with `dist/cronstable --version`. The **runner-native** jobs (`binaries-macos`, `binaries-windows`) install with **uv** (`uv venv` + `uv pip install` + `uv run pyinstaller`, using `astral-sh/setup-uv`). Every **container** job stays on **pip** inside its `docker run` containers, because uv's official image is amd64/arm64 only and it publishes no musl `ppc64le`/`s390x` wheels; pip is the arch-portable choice there:
   ...
   - **`binaries-macos`**: Builds Python 3.14 binaries for `arm64` on `macos-15` (Apple Silicon) and `amd64`/`amd64v3` on `macos-15-intel`. After the smoke test, `file` checks the native architecture to catch x86_64 binaries running through Rosetta on an ARM64 runner. Release assets use the names `cronstable-macos-{arm64,amd64,amd64v3}` and follow the signing and notarization policy described below. The separate `binaries-macos-experimental` job builds macOS 26 `arm64`/`amd64`/`amd64v3` binaries and a macOS 27 ARM64 binary on `xcode-27`. These advisory rows use `ship: false` and `continue-on-error`; they upload unsigned CI artifacts as `cronstable-macos26-{arch}` and `cronstable-macos27-arm64`. Both macOS 27 jobs verify the OS version and native architecture before testing or building.
   - **`binaries-windows`**: Windows, `amd64`/`amd64v3` on `windows-latest` and `arm64` on the `windows-11-arm` runner (both native; PyInstaller is not a cross-compiler). Built on Python 3.14 with the same `pyinstaller==6.22.1` pin and `dist/cronstable.exe --version` smoke test as the others. Any C-extension dep lacking a `win_arm64` wheel compiles from sdist with the runner's Visual Studio ARM64 toolchain.
     ...
     On a release, the `sign-windows` job signs and repackages all twelve artifacts (see [Windows signing](#windows-signing-azure-artifact-signing)). Release preflight requires the signing secrets; ordinary CI builds remain unsigned. Artifacts `cronstable-windows-{amd64,amd64v3,arm64,i686}.exe`, `.zip` and `.msi`. See [running on Windows](Running-on-Windows) and [Windows MSI](Windows-MSI).
...
7. **`docker-push`**: after `release`, copies the validated OCI images to GHCR and optional Docker Hub without rebuilding. Publish matrices retain `fail-fast: false` so a registry failure does not cancel other destinations.
...
failure. macOS 26 v3 remains CI-only under the existing non-gating policy.
...
`.github/docker-matrix.json` into the existing multi-arch image and an explicit
amd64v3 image using the same Dockerfile. Platform builders, OCI assembly and
...
- **`docker-push`** runs after `release`, verifies those archives and copies every platform with `skopeo copy --all --preserve-digests`. It publishes GHCR version/latest tags (Debian owns the bare tags, other distros retain their suffixes) and optional Docker Hub tags. It performs no image build or dependency resolution.

BuildKit caches are scoped per distro/platform and written only on trunk. Release dependency layers still refresh on each run. Optional cryptography builders always resolve fresh sources and reuse compiler work keyed by the source, base image and installed toolchain. The new compiler/source-wheel caches save only below 512 MiB per entry; best-effort maintenance limits these cache families on trunk to 2 GiB. Cache loss affects speed, while release publication uses immutable run artifacts. Platform OCI artifacts expire after two days for CI and seven days for releases.
