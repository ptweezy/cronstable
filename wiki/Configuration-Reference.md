...
| Option | Type | Default | Description |
| --- | --- | --- | --- |
| `name` | `Str` | required | Job identifier, unique across the whole assembled configuration (all files and includes together): the scheduler tracks jobs by name, so a duplicate is a `ConfigError` at load naming the colliding definitions. A job name can't start with `dag:`, the prefix of a [DAG's](Orchestration-and-DAGs) schedule job. Used in logs, the stream prefix, reports, statsd, and the HTTP API. |
