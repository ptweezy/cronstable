...
- **`docker`, `docker-glibc`, `docker-musl`** call the shared `build-docker.yml` workflow on every event. Each distro/platform builds independently; ARM64 uses native `ubuntu-24.04-arm` runners, amd64/386 use x64 runners, and other platforms use QEMU. Platforms with upstream wheels start immediately. Foreign platforms that need optional cryptography wheels wait only for their libc group. Each build exports an OCI artifact with the computed version and commit labels.
