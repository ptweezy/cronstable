...
- **Clustering and leader election.** yacron2 can now run as a cluster: point
  several replicas, deployed from the same configuration, at each other with a
  new optional top-level `cluster` section and they attest to one another's
  job set. Out of the box this is *observe-only*: every node still runs every
  job, and the dashboard and API simply show whether the replicas agree. Set
  `cluster.electLeader: true` and scheduled jobs become coordinated, so each
  job fires on exactly one node instead of once per replica. Election is
  quorum-gated: a node leads only while it can see a strict majority of the
  configured cluster agreeing with it, so a minority partition stands down
  (skips) rather than double-running, and two clean partitions can never both
  elect. Manual triggers (`POST /jobs/{name}/start`) always run on the node
  that received them, clustering only gates the *schedule*. Everything is
  opt-in: a configuration without a `cluster` section behaves exactly as
  before. The full guide, including the trust model, quorum math, failure
  windows, and sizing advice, is the new
  [Clustering and Leader Election](wiki/Clustering-and-Leader-Election.md)
  wiki page; the complete `cluster` schema is in the
  [Configuration Reference](wiki/Configuration-Reference.md).

- **Three pluggable leadership backends** (`cluster.backend`), all built on
  the core `aiohttp` dependency, so clustering adds **no new required
  dependency** on any of the supported architectures:
  - `gossip` (the default): embedded peer-to-peer attestation over mutual TLS
    (`cluster.listen`, `cluster.tls`, `cluster.peers`), with no external
    coordination service at all. Membership is pinned by a dedicated cluster
    CA; peers exchange job-set ids, cluster sizes, and coordination policy on
    a small `/peer` endpoint served on the separate mTLS listener (it is not
    part of the web API). Coordination is *best-effort*: a freshly cut
    partition or a thin-bridge topology can, in narrow windows of about one
    poll `interval`, skip or double-run a job, which the wiki spells out
    honestly. Certificate rotation is automatic: replace the cert/key/CA files
    on disk and the node picks them up in place, no restart needed.
  - `kubernetes`: a fenced `coordination.k8s.io/v1` Lease
    (`cluster.kubernetes.leaseName` and friends), giving exactly-once
    leadership while the apiserver is reachable. It talks to the apiserver
    through hand-rolled REST over `aiohttp` by default; installing the
    optional extra (`pip install yacron2[kubernetes]`) switches to the
    official client, and `clientLibrary: auto | library | http` pins the
    choice. In-cluster service-account auth and out-of-cluster `kubeconfig`
    are both supported.
  - `etcd`: a fenced lease-bound key (`cluster.etcd.endpoints`,
    `electionName`, `ttl`, optional `username`/`password`/`tls`) spoken
    directly to etcd's v3 gRPC-gateway JSON/HTTP API, so there is no etcd
    client library to install either.
  Both lease backends compute leadership from the monotonic clock, so an NTP
  or VM clock step can neither steal a fresh lease nor keep a stale leader
  alive, and a node whose renew loop stalls demotes itself *before* a peer
  becomes entitled to take over.

- **Per-job `clusterPolicy`** picks each job's failure trade-off under
  election: `Leader` (the default) runs at most once and *fails closed*, so if
  quorum is lost nobody runs it, right for non-idempotent work like billing or
  email; `PreferLeader` *never skips*, the lowest reachable node runs it even
  in a partition (accepting a possible double-run), right for idempotent
  must-happen work like cache refreshes; `EveryNode` runs on every node
  regardless, right for genuinely per-node work like local log rotation.
  The policy is inert without `electLeader`, and it is fingerprinted into the
  job-set id, so replicas that disagree about a job's policy show up as drift
  instead of silently coordinating differently. (Because the fingerprint
  gains a field, job-set ids change across this upgrade; the scheme stays
  `v1`, and ids are only meaningful between replicas on the same version.)

- **Spread the scheduled workload with `cluster.distribution: spread`**
  (gossip backend only; the default remains `single-leader`). Instead of one
  leader running every `Leader` job, each job is deterministically assigned an
  owner via rendezvous hashing over the live quorate members, so the work fans
  out roughly evenly across the cluster while keeping the exact same quorum
  gate and at-most-once behavior. The dashboard grows an **Owner** column and
  a per-peer **Owns** tally in spread mode, so the whole job-to-node
  distribution is visible at a glance.

- **Conflicts fail closed.** The gossip backend detects the misconfigurations
  that could otherwise elect two leaders, and pauses `Leader` jobs on every
  affected node until the configuration converges: a **duplicate `nodeName`**
  (two live instances claiming the same identity, told apart by a per-process
  instance id), a **cluster-size disagreement** (peers declaring different
  sizes mid-resize, which would let both old and new majorities elect), and a
  **coordination-policy mismatch** (peers running different `electLeader` /
  `distribution` settings). Transient mismatches during rolling deploys are
  debounced (`driftAfter`, default 3 rounds), and a fresh start holds
  `PreferLeader`'s never-skip gate closed for a short settling window until
  the peer view converges, so restarting a healthy cluster doesn't stampede
  every never-skip job on every node at once.

- **`@reboot` understands leadership.** Under `electLeader`, a `Leader` or
  `PreferLeader` `@reboot` job is deferred at boot until the cluster
  converges, then runs once on the node the policy selects, instead of once
  per replica. On the lease backends the "already ran" record is persisted in
  the Lease annotation (Kubernetes) or a sibling etcd key, keyed by job-set
  id, so a leadership failover does not re-fire it; changing the job set
  re-arms it.

- **The cluster is observable.** A new **`GET /cluster`** web endpoint returns
  the whole coordination picture as JSON: backend, node name, cluster size and
  quorum, elected leader and this node's role, per-peer statuses (`agreed`,
  `syncing`, `drifted`, `unreachable`, `untrusted`, `conflict`, `self`), and
  every conflict flag. `GET /jobs` gains a `clusterOwner` field in spread
  mode. The dashboard renders it all in a live **cluster panel**: a summary
  line with the agreement tally, quorum count, and role, color-coded per-peer
  status dots, a toggleable per-peer **swimlane timeline** that lets you watch
  a rolling deploy converge (`syncing` to `agreed`) or leadership move when a
  node dies, and a loud "Leader jobs paused" banner naming the conflict when
  the cluster is standing down.

- **The dashboard grows into a control room**, cluster or not:
  - a **column picker** in the jobs-table header: trim the classic columns or
    add new optional ones (`Policy`, `TZ`, `Next at`, success `Rate`), with
    the choice remembered per browser; the page widens to fit and falls back
    to horizontal scroll;
  - a **next-fire radar**: a fleet-wide "what fires in the next ten minutes"
    feed with a timeline track;
  - an **activity heatmap**: a per-job run-outcome punchcard over a selectable
    window;
  - a **wallboard / TV kiosk mode**: a full-screen status-tile grid with a
    footer clock and tallies, a **zen screensaver** that takes over when the
    fleet is all-clear, and an **adaptive audible alarm** that escalates while
    failures stand and can be acknowledged;
  - an **incident cockpit**: a fleet verdict bar (hidden while healthy) that
    opens a correlated-failure timeline and a **mitigate console** for acting
    on it;
  - a **cron sandbox**: type any schedule expression and see its upcoming
    fires in a chosen timezone frame;
  - an opt-in **run ledger**: long-horizon run history and slow-run detection
    accumulated in the browser (IndexedDB);
  - a fourth theme, **Carolina Blue** CRT phosphor, a BIOS-style boot
    self-test (toggleable), and honest staleness: when polls stop landing,
    panels dim and say so instead of freezing on the last healthy frame.

- **Runnable examples for every backend.** `example/cluster` (a three-node
  mutual-TLS gossip cluster with cert generation), `example/cluster-large` (a
  ten-node `spread` demo), `example/etcd` and `example/kubernetes` (each lease
  backend end-to-end, the latter with the Lease RBAC manifests),
  `example/acme-platform` (a five-node "ops platform" with realistic
  workloads, reporting, and metrics), and `example/zen-demo` (a single calm
  node for the wallboard), each with a matching `docker-compose-*.yml` at the
  repo root. A new `.gitattributes` pins the example shell scripts to LF so
  the demos survive a Windows clone.

- **A failed job spawn can no longer kill the scheduler.** Spawn-time
  `OSError`s (fd exhaustion `EMFILE`/`ENFILE`, `ENOMEM`, `EPERM`/`EACCES`,
  fork-limit `EAGAIN`) raised by `create_subprocess_exec` are not
  `SubprocessError` subclasses and used to propagate out of the scheduling
  path and bring the daemon down; they are now caught like other launch
  failures, the run is marked `start_failed`, and the retry logic takes it
  from there.

- The PyPI one-liner gained a few adjectives it can now back up.

- Internal: the clustering subsystem ships with extensive new test suites
  (cluster manager and quorum logic, per-job ownership, both lease backends,
  and cluster config validation), packaged as a new `yacron2.backends`
  package. The dev extra adds `cryptography` (used only to mint throwaway
  certificates for the mTLS tests; it ships no win-arm64 wheel, so it is
  skipped there and those tests self-skip).
