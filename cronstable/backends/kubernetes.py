...
class KubernetesBackend(StoreLeaseBackend):
    ...
    async def _persist_reboot_ran(self) -> None:
        ...
        *before* launching it, and both the cron and leadership docstrings
        ...
        :class:`~cronstable.leadership.LeaseBackend` ``mark_reboot_ran`` only
...
class _K8sHttpTransport(_K8sTransport):  # pragma: no cover - network I/O
    ...
    def _load_kubeconfig(self, path: str) -> None:
        ...
        if (
            ...
            raise ConfigError(
                ...
                "native client (pip install 'cronstable[kubernetes]') so the "
    ...
    def _material(
        ...
        if path:
            return path
        if data:
            fd, tmp = tempfile.mkstemp(prefix="cronstable-k8s-")
