...
# extract_deps.py helper (the core dependencies plus the push-pq and discovery
...
#
# The helper resolves the cryptography line itself: a PEP 508 marker reads
...
# are part of the image contract: a `push:` or `web.bonjour` section fails
# closed at config load, so an image missing them would crash-loop with no
# in-image remediation (pip is stripped from the runtime stage below).
# Verify with a real sealed-box round-trip (catches a QEMU-miscompiled
# libsodium on the source-built arches) plus a zeroconf import; a failure
# fails the build. cryptography carries the post-quantum `xwing` suite and
# is present only where docker/extract_deps.py resolved it, so it is checked
# only there, and here only for age: a release too old to carry the HPKE
# module. Whether it really seals is proved after the project install below,
# by the daemon's own `--sealable-suites`.
...
# Prove the installed daemon seals `xwing` wherever the dependency layer
# resolved cryptography. The extras check above proves only that the HPKE
# module is findable; a wheel whose bundled OpenSSL lacks ML-KEM passes
# that and fails at the first pairing. `--sealable-suites` seals one real
# probe through the daemon's own path, the same gate the release binaries
# pass on the frozen artifact. An image the helper resolved without
# cryptography seals x25519 only and skips this.
RUN set -eux; \
