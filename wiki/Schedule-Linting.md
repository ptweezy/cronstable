...
| `dst-skipped-time` | note | The scheduled wall time falls in a spring-forward gap of the job's time zone (with the actual date, such as `2027-03-14` for `America/New_York`). The run is *not* lost: it fires at the shifted wall time. |
...
The DST rules need a resolvable zone, so they run only for jobs with an explicit `timezone:`. A fixed-offset frame like UTC never transitions, and the daemon cannot see the DST rules behind a bare local clock. The rules are also skipped for schedules with unrestricted hours, which fire straight through a transition with nothing to call out.
...
`GET /schedule/preview?expr=<expression>&tz=<zone>&count=<n>` parses, describes, previews, and lints any expression with the daemon's own engine, the same engine the sandboxes use. See [HTTP API](HTTP-API) for the full response shape. AI agents get the same payload through the `cron_validate_schedule` and `cron_explain_schedule` [MCP tools](MCP).
