...
(Earlier releases accepted `year` in the schema but silently dropped it when building the crontab string, so it had no effect; it is now honored, as documented above and in `README.md`.)
...
- On each wake, `spawn_jobs` iterates all jobs and calls `job_should_run`. For a `CronTab` job it evaluates `crontab.test(schedule_slot(job))`, where `schedule_slot` truncates the current time (in the job's timezone) to that job's resolution: the whole **second** for a second-level job, or the top of the **minute** otherwise.
