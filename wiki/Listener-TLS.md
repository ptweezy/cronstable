...
Use `web.tls` when the dashboard or the API is reachable from anywhere that is
not this host: another machine on the network, a colleague's laptop, a CI
runner, a desktop MCP client. The API carries the bearer token
(`web.authToken`) in an `Authorization` header. On a routable interface, a
...
`IP:::1` is the IPv6 loopback (the `IP:` prefix followed by `::1`). If you only
ever dial the host by address, drop the `DNS:cronstable.internal` entry. Add
...
https://cronstable.internal:8443/status`. A browser shows a trust warning until
the CA is added to the operating system or browser trust store.
...
The daemon builds one context per app start and shares it across every
`https://` site, so all of them serve the same certificate.
...
**Whether the files exist is deliberately not checked at parse time.**
Configuration parsing touches no filesystem, `--validate-config` is often run
somewhere that is not the deployment target, and a Kubernetes-mounted secret
need not exist yet at first boot. The files are read when the listener starts.
If they do not load, the web API does **not** start; it is never downgraded to
plaintext. The daemon logs the error and retries on the next config reload,
...
On every `https://` connection, the client must now present a certificate that
chains to `client-ca.pem`. A client that presents nothing is refused during the
handshake, and so is a certificate from any other CA. There is no middle
setting: "optional client CA" means the *key* is optional, never that a
presented certificate is. A mode that completed the handshake for a client
presenting nothing would turn `clientCa` into a no-op for the operator who set
it precisely to authenticate callers.

**The CA file is the caller allowlist.** A server cannot do hostname
verification against a client, so the listener accepts *any* certificate that CA
has ever signed, with no further check on which one it is. Point `clientCa` at a
**dedicated CA that issues to cronstable clients only**, never at a shared
service-mesh or organization-wide CA. With an organizational CA, every workload
...
mTLS-terminating proxy. Therefore `mcp.enabled` on an `https://` listener with
...
With `mcp.enabled` set, the MCP token gate reads the *routable* addresses. A
mixed list like this needs `web.authToken` (or `clientCa`) because of the
`10.0.0.5` entry, even though the loopback entry alone would not. Without
`mcp.enabled`, no listener requires a token.
...
TLS at all. When jobs run **off this host** and dial it across the network, it
needs TLS, because it serves per-run bearer tokens and staged job secrets.
...
| `--insecure` | `CRONSTABLE_WEB_INSECURE` (`1`/`true`/`yes`) | Skip verification entirely. The client warns on stderr, because it still sends the bearer token to whoever answers the connection. |
...
* **The in-job CLI takes none of them.** It reads `CRONSTABLE_STATE_CACERT` from
  ...
  [`CRONSTABLE_STATE_CACERT`](#cronstable_state_cacert) earlier.
...
Vault, and a Kubernetes secret refresh renew) leaves the configuration bytes
identical, so the ordinary "did the config change?" check never fires for it.
Meanwhile the SSL context, built once at listener start, holds the old
certificate in memory. Without a dedicated check, the daemon keeps serving the
expired certificate.

The daemon notices it by fingerprint. On each housekeeping pass (at most once
...
changed, it **restarts** the web app so the context is rebuilt from the new
material. `os.stat` follows symlinks, so the daemon picks up the atomic symlink
swap behind a Kubernetes mounted secret.
...
starting`.** The daemon could not load the certificate or key, so the whole web
API stays down rather than any port being served in cleartext. The rest of the
line carries the underlying error: a wrong path, a key that does not match the
certificate, or a file the daemon's user cannot read. Fix it and the next config
reload retries. No restart is needed.

**The log says `Ignoring web listen url ...: no usable web.tls material for an
https listener`.** The daemon skipped a single `https://` address because no
context was available for it. Parse-time validation normally makes this
unreachable (an `https://` listen with no `web.tls` is a `ConfigError`), so in
practice it means `web.tls` is present but empty. Give it a `cert` and `key`, or
make the address `http://`.

**The log says `web: new TLS material is not yet loadable`.** The daemon
detected a rotation, but the new files do not load yet, typically a renewal
caught half-written. The old listener is still up and still serving, and the
check repeats on the next reload. If the message persists past a couple of
minutes, the new material is genuinely broken rather than half-written. Check
the renewal job and that the certificate and key are a matching pair.

**An mTLS listener refuses a client during the handshake.** With `clientCa` set,
the listener rejects a client that sends no certificate, or one signed by a
different CA, before any HTTP request happens, so there is no status code to
read, only a handshake error. Confirm the client is passing both `--client-cert`
and `--client-key`, and that the certificate chains to the exact CA in
`clientCa`:
...
- [Clustering and Leader Election](Clustering-and-Leader-Election): `cluster.tls`, a separate always-mutual block for the peer mesh, unaffected by anything on this page.
