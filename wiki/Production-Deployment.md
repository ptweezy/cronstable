...
> demonstration. It `pip install`s cronstable and is **not** non-root. For
> production, use the published GHCR image (or base your own image on it,
> described later), not the example.
...
published image. Your image inherits the non-root user, entrypoint, and config
path:
...
The published image satisfies a fully restricted pod security context:
...
* **`readOnlyRootFilesystem`**: neither the image nor a `pip`/`pipx` install
  requires runtime writes. Mount the crontab read-only. See [writable-path
  ...
  binary's self-extraction temp directory (only if you deploy the binary).
* **`fsGroup`-mounted config/secrets**: mount config and secret volumes with an
  `fsGroup` (for example, `65534`) so the non-root process can read them.
...
* **`backend: gossip` (the default, no coordination store).** A quorum-gated,
  ...
  * **Use an odd replica count.** 3 replicas tolerate one failure, 5 tolerate
    ...
    than 1: a majority of 2 is 2, so both must be up to run anything. The daemon
...
* **Use a headless `Service` for peer DNS.** Pair a gossip StatefulSet with a
  ...
  per-ordinal DNS name (`cronstable-0.<svc>.<ns>.svc`, `cronstable-1.<svc>…`). You
  list those names under `cluster.peers`, and the certificate SANs must match
  them. A lease backend needs none of this: it has no peer listener.
...
alert on: a node that cannot see a majority, or that on a lease backend cannot
reach the store, stands its `Leader` jobs down. For the full signal set, see the
...
> membership allowlist (explained later), so a certificate issued for a
> dashboard user must not chain to it. Provision each from its own issuer. For
> the listener side, see [listener TLS](Listener-TLS).

This applies to the `gossip` backend only (the lease backends use no per-node
mTLS certs). Day-to-day operation needs little intervention: cronstable reloads
its mTLS contexts **in place**. On each config-reload pass it compares the
on-disk `ca`/`cert`/`key` against what it loaded, and if any changed it
rebuilds the TLS contexts within ~1 minute, with **no restart**. It dry-runs
the new material first, so a half-written file is retried, not fatal. It
therefore picks up in-place renewals from cert-manager, Vault, and
mounted-secret refreshes (same paths, new bytes) automatically. The Clustering
page covers the full mechanism under
[certificate rotation](Clustering-and-Leader-Election#certificate-rotation); the
operational cases:

* **The cluster `ca` is the membership allowlist.** The daemon trusts *any* cert
  the configured `ca` signs to assert a node identity and gossip state, so the
  `ca` **must** be a dedicated, single-purpose CA issued only to cronstable
  nodes, **not** a shared service-mesh or organization-wide CA. Any cert the CA
  admits could otherwise fabricate the `/peer` payload (fake agreement, trip the
  conflict gate, or suppress an `@reboot` job). Provision leaf certs from that
  dedicated PKI (a private cert-manager issuer, an internal CA). The daemon
  only consumes them.
* **Rotating a single node's cert.** If a leaf your PKI renews still chains to
  the same `ca`, rotating it needs no coordination: write the new `cert`/`key`
  in place, the node reloads within ~1 minute, and its peers keep trusting it.
  Issue the replacement with a comfortable overlap (well before the old cert
  expires) so a slow refresh never leaves the node serving an expired cert.
* **Rolling the cluster CA.** Changing the CA itself needs care, because a node
  ...
  3. Re-issue each node's leaf cert from the **new** CA, **one node at a time**,
     ...
     return to `agreed`.
  4. After every node presents a new-CA cert, narrow the bundle back to the
...
* **Recovering from an `untrusted` cascade.** If peers start showing `untrusted`
  ...
  rotation, certs and CA trust have diverged. The usual causes are a CA roll
  that skipped the overlap step, or a node whose refresh lagged. Recovery needs
  **no restart**: restore the trust overlap (push a CA bundle covering
  whichever CA the still-`untrusted` peers were issued from), or finish rolling
  the lagging nodes onto the new CA. Each node reloads within ~1 minute and
  peers return to `agreed` automatically. `Leader` jobs stand down (fail closed)
  while quorum is lost, so the cascade **skips** firings rather than
  double-running them: no split-brain during recovery, only missed firings
  until trust reconverges.

### Production clustering checklist

Before scaling past `replicas: 1`, work through this checklist for your backend.
Each row links to the authoritative section on the Clustering page.
...
RFC1123 `leaseName`/`leaseNamespace` shapes, the etcd endpoint/TLS rules, and
the `electLeader` 2-node rejection. See
...
> Windows does not support `unix://` web listeners (the Proactor event loop
> lacks `create_unix_server`). The daemon skips such a listen URL with the
> warning `Ignoring web listen url <url>: unix-socket listeners are not
> supported on this platform`. Use an `http://` listener instead.
> `web.socketMode` only applies to unix sockets, so it is irrelevant on Windows.
...
read-only root filesystem makes `/tmp` fail that test. For the error messages
and the Docker `--tmpfs` `exec` recipe, Kubernetes `emptyDir` mount, and
...
* **Shutdown**: on POSIX, cronstable installs handlers for `SIGINT` and
  `SIGTERM` and shuts down gracefully, so the default pod termination path needs
  no extra configuration. On Windows (where the Proactor loop has no
  `add_signal_handler`) it instead handles `SIGINT`/`SIGBREAK` with
  `signal.signal` plus a heartbeat timer, and console close or OS shutdown with
  a native console-control handler. Logoff is excluded deliberately: a service
  receives it for every user's sign-out. Either way, the daemon finishes the
