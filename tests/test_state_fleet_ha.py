...
# Most tests build their cron through the ``fleet_cron`` fixture below,
# which layers this file's teardown extras onto the shared ``stateful_cron``
# factory (tests/conftest.py).  The path-taking helpers remain for
# test_claim_declines_ineligible_ladders, whose per-case stores live under
# distinct tmp_path sub-paths that the shared factory does not take.
...
async def _cancel_fleet_tasks(cron):
...
async def _stop_state(cron):
    await _cancel_fleet_tasks(cron)
...
@pytest.fixture
async def fleet_cron(stateful_cron):
    """The shared ``stateful_cron`` factory plus this file's teardown extras.

    ``make(yaml, extra_state=...)`` matches the shared factory's shape;
    ``resume_host="node-a"`` additionally prepends ``topology: shared`` and
    activates cross-node retry resume the way ``_resume_cron`` does.  This
    fixture's own teardown runs ``_cancel_fleet_tasks`` per cron (newest
    first) and, by fixture finalization order, runs BEFORE the shared
    factory's drain-and-stop, preserving ``_stop_state``'s sequence.
    """
    crons = []

    async def make(yaml, *, extra_state="", resume_host=None):
        if resume_host is not None:
            extra_state = "  topology: shared\n" + extra_state
        cron = await stateful_cron(yaml, extra_state=extra_state)
        if resume_host is not None:
            cron._state_host = resume_host
            cron._elect_leader_configured = True
            cron.cluster_manager = _stub_manager()
            assert cron._retry_resume_active() is True
        crons.append(cron)
...
async def test_slot_claimed_and_released_around_a_run(fleet_cron):
    cron = await fleet_cron(_FORBID_JOB)
...
async def test_forbid_skips_when_foreign_holder_is_live(fleet_cron):
    cron = await fleet_cron(_FORBID_JOB)
    backend = cron.state_backend
...
async def test_replace_pursuit_cancels_and_relaunches(fleet_cron):
    ...
    cron = await fleet_cron(
        _REPLACE_JOB, extra_state="  slotTtlSeconds: 6\n"
    )
    backend = cron.state_backend
    foreign = await backend.acquire_lease("slots/j", "node-b#feed", 60)
    assert await cron.maybe_launch_job(cron.cron_jobs["j"]) is False
...
async def test_stale_cancel_records_are_inert(fleet_cron):
    ...
    cron = await fleet_cron(
        _REPLACE_SLEEPER, extra_state="  slotTtlSeconds: 5\n"
    )
    backend = cron.state_backend
...
async def test_holder_observes_cancel_and_yields(fleet_cron):
    cron = await fleet_cron(
        _REPLACE_SLEEPER, extra_state="  slotTtlSeconds: 5\n"
    )
    backend = cron.state_backend
...
async def test_slot_unavailable_degrade_vs_fail_closed(fleet_cron):
...
async def test_slot_sick_store_follows_policy(monkeypatch, fleet_cron):
...
async def test_slot_lock_fidelity_latch(monkeypatch, fleet_cron):
...
async def test_reconcile_closes_a_crashed_run(monkeypatch, fleet_cron):
    cron = await fleet_cron(_PLAIN_JOB)
...
async def test_reconcile_preserves_catchup_for_onmissed_jobs(
    ...
    cron = await fleet_cron(_CATCHUP_JOB)
    monkeypatch.setattr(platform_mod, "pid_alive", lambda pid: False)
...
async def test_reconcile_skips_when_pid_still_alive(monkeypatch, fleet_cron):
...
async def test_slot_takeover_reconciles_foreign_open_record(fleet_cron):
...
async def test_abandon_hands_off_on_shared_store(fleet_cron):
    cron = await fleet_cron(_RETRY_JOB, resume_host="node-a")
...
async def test_handoff_is_claimed_and_fires_elsewhere(fleet_cron):
    giver = await fleet_cron(_RETRY_JOB, resume_host="node-a")
    taker = await fleet_cron(_RETRY_JOB, resume_host="node-b")
    state = JobRetryState(1, 2, 60)
    state.next_delay()
...
async def test_claim_respects_the_staleness_grace(fleet_cron):
    taker = await fleet_cron(_RETRY_JOB, resume_host="node-b")
    fresh = {
        "kind": "pending",
        "attempt": 1,
...
async def test_claim_settles_superseded_by_run(fleet_cron):
...
async def test_consume_aborts_when_ladder_was_claimed_away(fleet_cron):
    ...
    owner = await fleet_cron(_RETRY_JOB, resume_host="node-a")
    state = JobRetryState(1, 2, 60)
    state.next_delay()
...
async def test_consume_unchanged_without_resume(fleet_cron):
    ...
    cron = await fleet_cron(_RETRY_JOB)
...
async def test_concurrent_claims_yield_exactly_one_owner(fleet_cron):
...
async def test_phantom_release_cleans_a_true_phantom(fleet_cron):
...
async def test_claim_dropped_when_local_ladder_armed_during_claim(fleet_cron):
...
async def test_inflight_open_sorts_before_close_for_a_fast_run(fleet_cron):
