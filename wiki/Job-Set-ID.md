...
* **Independent of job order**, of how the jobs are split across
  ...
  inline or hoisted into a `defaults` block: the fingerprint is taken over the
  merged result only. Numeric spellings are normalized, so an inline
...
* **It covers every behavior-affecting field**, so any meaningful change to a
  ...
  * `shell`, `concurrencyPolicy`, `clusterPolicy`, `captureStdout` /
    `captureStderr`, `streamPrefix`, `saveLimit`, `maxLineLength`.
  * The effective scheduling time zone: the resolved `utc` / `timezone` frame,
    so behaviorally identical spellings match.
  * `enabled`, `onlyIfLastSucceeded`, `failsWhen`.
  * `onFailure` / `onPermanentFailure` / `onSuccess`, including the retry and
    reporting policy, with secret values redacted.
  * The sorted *names* of `environment` variables.
  * `executionTimeout`, `killTimeout`, `statsd`, `user`, `group`.
  * `concurrencyScope` when set to `cluster`.
...
* The catch-up trio (`onMissed`, `startingDeadlineSeconds`,
  `catchupJitterSeconds`) and the archival pair (`archiveOutput`,
  `redactArchivedSecrets`), which are restart-time or observability-only,
  node-local behavior.
* `workingDirectory`, for the same reason environment *values* are excluded. It
...
* Everything outside the job definitions, in particular the `cluster` section
  ...
  config never move the id.

Fields added after the `v1` scheme shipped enter the identity only when they
are set away from their default (`concurrencyScope`, listed earlier, is one),
so upgrading cronstable never changes the id of an existing configuration.
...
* **Inline reporting secrets are redacted.** A Sentry DSN, mail password, or
  ...
  (`value` / `fromFile` / `fromEnvVar`, and the file path or environment
  variable name) is still part of the identity. The literal secret never is.
* **Only the names of `environment` variables are hashed**, never their
  values: environment variables commonly carry secrets, and a per-host value
  (for example, from `env_file`) would otherwise make identical configs
  fingerprint differently across hosts. The name set *does* include names
  contributed by `env_file`, so replicas must ship environment files with the
  same variable names (only the values may differ per host).

Two consequences: the id is **safe to log and serve**, and **rotating a secret
or changing an environment value does not change it**.
...
no debounce, because such ids are not comparable (see
...
`cmd.exe` on Windows. Unless `shell` is set explicitly, the same YAML
fingerprints differently across platforms. Compare instances running on the
same platform, which is what replicas are.

## Where it surfaces

* **CLI**: `cronstable --job-set-id` prints the id to stdout and exits, for
  scripts and deploy checks. See the [command-line reference](CLI-Reference).
...
* **Cluster**: the id is the `job_set_id` field on `GET /cluster` and the
  ...
  so a changed job set re-arms leader-gated `@reboot` one-shots. What the
  cluster does with the id is covered in
...
serialized to a canonical JSON form and SHA-256 hashed. The per-job digests are
then sorted, which neutralizes order. The sorted list is hashed together with
the scheme version to yield the final `v1:<64 hex>` id. An empty job set yields
