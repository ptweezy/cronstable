...
The `.github/workflows/release.yml` workflow (named `CI`) runs on every `push` (any branch) and every `pull_request`. The separate `nightly` workflow runs scheduled checks. For ordinary commits, `CI` runs static checks and the required test matrix before starting the parallel binary and image builds. This keeps long emulated builds from occupying workers while basic tests remain queued. Only a release (described later) proceeds to publish.
