...
Supported syntax (see ``wiki/Schedules-and-Timezones.md``):
...
- ``?`` alone in either day field reads as ``*`` (Quartz); anywhere else
  an error. Unsupported Quartz forms include a dialect hint.
...
class CronTab:
    ...
    def next(
        ...
        # For aware datetimes, return the delay to the first UTC instant
        # that occurrences would yield after now.
        now_utc = now.astimezone(_UTC)
        instant = self._next_instant(now, now_utc)
        if instant is None:
            return None
        return (instant - now_utc).total_seconds()
    ...
    def _next_instant(
        ...
        :meth:`occurrences`. Resolve each matching local time with ``fold=0``
        and compare the resulting UTC instants. A match in a daylight saving
        time gap shifts forward by the gap's duration. A repeated local time
        runs only on its first occurrence.

        The search can start before the local time in ``now``; see
        ``_gap_rewound_seed``. Comparing UTC instants prevents returning a time
        that has already passed.

        During a forward clock transition, shifted matches can overlap with
        valid local times after the gap. Compare the first shifted match with
        the first valid match after the gap and return whichever occurs first.
        During Lord Howe's 30-minute transition, ``0,20,40 2`` runs at 02:30,
        02:40, and 02:50. The valid 02:40 match falls between shifted matches.
        """
        tz = now.tzinfo
        ...
        while True:
            ...
            resolved_civil = resolved_utc.astimezone(tz).replace(tzinfo=None)
        ...
        # If now is already past the gap, start the search from its displayed
        # local time. Convert through UTC because now itself might represent
        # a nonexistent local time.
        low, high = target, resolved_civil
        while high - low > _ONE_SECOND:
            mid = low + (high - low) // 2
            mid = mid.replace(microsecond=0)
            back = mid.replace(tzinfo=tz).astimezone(_UTC).astimezone(tz)
            if back.replace(tzinfo=None) == mid:
                high = mid
            else:
                low = mid
    ...
    def occurrences(
        ...
        if type(tz) is datetime.timezone:
            ...
            # over, no fold to honor, no instant to re-render.
        ...
        if tz is None:
            while True:
                ...
                yield target
                civil = target
        # For aware datetimes, yield the same instants as repeated calls to
        # _next_instant. last_utc honors start.fold, so a fold=1 start never
        # repeats a run from the first occurrence of that local time.
        # Resolve local matches directly when their order matches UTC order.
        # Use _next_instant around forward clock transitions, when shifted
        # matches can still be pending or the next match falls in a gap.
        # _gap_rewound_seed identifies pending shifted matches.
        civil = start.replace(tzinfo=None)
        last_utc = start.astimezone(_UTC)
        last = start
        careful = self._gap_rewound_seed(start, last_utc) != civil
        while True:
            if careful:
                instant = self._next_instant(last, last_utc)
                if instant is None:
                    return
                last, last_utc = instant.astimezone(tz), instant
                yield last
                civil = last.replace(tzinfo=None)
                careful = self._gap_rewound_seed(last, last_utc) != civil
                continue
            target = self._next_civil(civil)
            if target is None:
                return
            # Resolve with fold=0 and convert through UTC to get the local
            # time displayed at that instant.
            resolved_utc = target.replace(tzinfo=tz).astimezone(_UTC)
            resolved = resolved_utc.astimezone(tz)
            resolved_civil = resolved.replace(tzinfo=None)
            if resolved_utc <= last_utc:
                # This instant has already passed, for example during a
                # repeated local hour. Advance without yielding, matching
                # _next_instant.
                civil = max(target, resolved_civil)
                continue
            if resolved_civil != target:
                # A match in a gap: valid local times might occur first.
                careful = True
                continue
            yield resolved
            last, last_utc = resolved, resolved_utc
            civil = resolved_civil
